...
def _article_out(article: Article, source_name: str) -> ArticleOut:
    return ArticleOut(
        id=article.id,
...
@router.get("/articles/favorites", response_model=FeedPage)
def get_favorites(
    cursor: str | None = Query(default=None),
    limit: int = Query(default=20, ge=1, le=50),
    user: User = Depends(get_current_user),
    ...
    """La carpeta de favoritas del usuario, con la misma paginación del feed."""
    stmt = (
        select(Article, Source.name)
        .join(Source, Article.source_id == Source.id)
        .where(Source.user_id == user.id)
...
def favorite_article(
    ...
    """Marca o desmarca una favorita. Al marcarla se genera el informe extenso
    (si no existía) y se extraen más fotos del artículo, en la misma pasada.
    Al desmarcarla solo se quita de la carpeta: informe y fotos quedan cacheados."""
    row = session.exec(
    ...
    return _article_out(article, source.name)
